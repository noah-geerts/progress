...
public class SessionControllerIntegrationTests {
  ...
  void setup() {
    ...
    seededSets = List
        .of(
            // Bench 225x5
            PerformedSet.builder().reps(5).weight(225.0).performedExercise(seededPEs.get(0)).uid(TEST_UID).build(),
            // Bench 220x5
            PerformedSet.builder().reps(5).weight(220.0).performedExercise(seededPEs.get(0)).uid(TEST_UID).build(),
            // Dumbell Press 60sx12
            PerformedSet.builder().reps(12).weight(60.0).performedExercise(seededPEs.get(1)).uid(TEST_UID).build(),
            // Squat 315x3
            PerformedSet.builder().reps(3).weight(315.0).performedExercise(seededPEs.get(2)).uid(TEST_UID).build(),
            // Squat 315x2
            PerformedSet.builder().reps(2).weight(315.0).performedExercise(seededPEs.get(2)).uid(TEST_UID).build());
  ...
  class GetSession {
    ...
    void shouldReturnCorrectChestDayWorkout_WhenGettingChestDay() throws Exception {
      ...
      mockMvc.perform(get("/sessions/2025-01-01")
          ...
          .andExpect(jsonPath("$.performedExercises[0].exercise.name").value("Bench Press"))
          .andExpect(jsonPath("$.performedExercises[0].sets", hasSize(2))) // 2 bench press sets
          .andExpect(jsonPath("$.performedExercises[0].sets[0].weight").value(225.0))
          .andExpect(jsonPath("$.performedExercises[0].sets[0].reps").value(5))
          .andExpect(jsonPath("$.performedExercises[0].sets[1].weight").value(220.0))
          .andExpect(jsonPath("$.performedExercises[0].sets[1].reps").value(5))

          // Check second performed exercise (Dumbell Press)
          .andExpect(jsonPath("$.performedExercises[1].exercise.name").value("Dumbell Press"))
          .andExpect(jsonPath("$.performedExercises[1].sets", hasSize(1))) // 1 dumbell press set
          .andExpect(jsonPath("$.performedExercises[1].sets[0].weight").value(60.0))
          .andExpect(jsonPath("$.performedExercises[1].sets[0].reps").value(12));
    }
    ...
    void shouldReturnCorrectLegDayWorkout_WhenGettingLegDay() throws Exception {
      ...
      mockMvc.perform(get("/sessions/2025-01-02") // Fixed: Leg Day should be on Jan 2
          ...
          .andExpect(jsonPath("$.performedExercises[0].sets[0].reps").value(3))
          .andExpect(jsonPath("$.performedExercises[0].sets[1].weight").value(315.0))
          .andExpect(jsonPath("$.performedExercises[0].sets[1].reps").value(2));
    }
  ...
  class CreateSession {
    ...
    void shouldCreateSession_WhenSessionDoesNotExist() throws Exception {
      ...
      String requestBody = objectMapper
          .writeValueAsString(SessionRequestDto.builder().name("New Chest Session").build());

      // Act & Assert (don't fetch for 2025 jan 1 or 2)
      mockMvc
          .perform(post("/sessions/2025-01-03").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID)))
              .contentType("application/json").content(requestBody))
          .andExpect(status().isCreated()).andExpect(jsonPath("$.date").value("2025-01-03"))
    ...
    void shouldReturnConflict_WhenSessionExists() throws Exception {
      ...
      mockMvc.perform(post("/sessions/2025-01-01").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID)))
          .contentType("application/json").content(requestBody)).andExpect(status().isConflict());
    }
  ...
  class UpdateSession {
    ...
    void shouldReturnUpdatedSession_WhenSessionExists() throws Exception {
      // Arrange
      String requestBody = objectMapper
          .writeValueAsString(SessionRequestDto.builder().name("Chest Day Modified").build());

      // Act & Assert (modify jan 1 2025, which already exists)
      mockMvc
          .perform(patch("/sessions/2025-01-01").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID)))
              .contentType("application/json").content(requestBody))
          .andExpect(status().isOk()).andExpect(jsonPath("$.name").value("Chest Day Modified"))
          .andExpect(jsonPath("$.date").value("2025-01-01"))
    ...
    void shouldGiveNotFound_WhenSessionDoesNotExist() throws Exception {
      ...
      mockMvc
          .perform(patch("/sessions/2025-01-03").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID)))
              .contentType("application/json").content(requestBody))
          .andExpect(status().isNotFound());
    }
    ...
    class DeleteSession {
      ...
      void shouldReturnNocontent_WhenSessionExists() throws Exception {
        ...
        mockMvc
            .perform(delete("/sessions/2025-01-01").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID))))
      ...
      void shouldGiveNotFound_WhenSessionDoesNotExist() throws Exception {
        ...
        mockMvc
            .perform(delete("/sessions/2025-01-03").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID))))
            .andExpect(status().isNotFound());
      }
    }
    ...
    class Workflows {
      ...
      void shouldReceiveUpdatedSession_WhenCreatingThenUpdating() throws Exception {
        // Arrange
        String requestBody = objectMapper
            .writeValueAsString(SessionRequestDto.builder().name("New Session").build());
        ...
        requestBody = objectMapper
            .writeValueAsString(SessionRequestDto.builder().name("Updated Name").build());
        mockMvc
            .perform(patch("/sessions/2025-01-03").with(jwt().jwt(jwt -> jwt.claim("sub", TEST_UID)))
                .contentType("application/json").content(requestBody))
            .andExpect(status().isOk());
        ...
        Optional<Session> session = sessionRepo.findByDateAndUid(LocalDate.of(2025, 1, 3), TEST_UID);
