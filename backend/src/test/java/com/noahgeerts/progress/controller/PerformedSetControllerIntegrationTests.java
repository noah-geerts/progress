...
public class PerformedSetControllerIntegrationTests {
    ...
    class CreatePerformedSet {
        ...
        void shouldReturnCreatedAndCreateInDB_whenRequestValid() throws Exception {
            ...
            String requestBody = objectMapper.writeValueAsString(
                    CreatePerformedSetDto.builder().peid(peid).position(position).reps(reps).weight(weight).build());

            // Act
            MvcResult result = mockMvc
                    .perform(post("/sets").with(createTestJWT()).contentType("application/json").content(requestBody))
                    .andExpect(status().isCreated()).andExpect(jsonPath("$.position").value(position))
                    .andExpect(jsonPath("$.reps").value(reps)).andExpect(jsonPath("$.weight").value(weight))
                    .andReturn();
        ...
        void shouldReturnConflict_whenSetAlreadyExistsWithPositionAndPeid() throws Exception {
            ...
            // Act
            mockMvc.perform(post("/sets").with(createTestJWT()).contentType("application/json").content(requestBody))
        ...
        @ParameterizedTest
        @ValueSource(strings = {
                "{\"peid\": 1, \"position\": 1, \"reps\": 1, \"weight\": \"shouldn't be a string\"}", // weight wrong
                                                                                                      // datatype
                "{\"peid\": 1, \"position\": 1, \"reps\": \"string\", \"weight\": 225.0}", // reps wrong datatype
                "{\"peid\": 1, \"position\": \"hi\", \"reps\": 1, \"weight\": 225.0}", // position wrong datatype
                "{\"peid\": \"hi\", \"position\": 1, \"reps\": 1, \"weight\": 225.0}", // peid wrong datatype
                ...
                "{\"peid\": 1, \"position\": 1, \"reps\": 1, \"weight\": 225.0, \"extra\": 0}" // extra field
        })
    ...
    class UpdatePerformedSet {
        ...
        void shouldReturnOkAndUpdateInDB_whenRequestValid() throws Exception {
            ...
            // Act
            mockMvc.perform(
                    patch("/sets/" + stid).with(createTestJWT()).contentType("application/json").content(requestBody))
        ...
        void shouldReturnBadRequest_whenRequestBodyDoesNotMatchDto(String requestBody) throws Exception {
            mockMvc.perform(
                    patch("/sets/" + seededSets.get(0).getStid()).with(createTestJWT()).contentType("application/json")
                            .content(requestBody))
                    .andExpect(status().isBadRequest());
        }
    }
    ...
    class DeletePerformedSet {
        ...
        void shouldReturnNoContentAndDeleteInDB_whenRequestedSetExists() throws Exception {
            ...
            // Act
            mockMvc.perform(
                    delete("/sets/" + stid).with(createTestJWT()))
                    .andExpect(status().isNoContent());

            // Assert (make sure it was deleted)
            Optional<PerformedSet> deletedInDB = setRepo.findById(stid);
